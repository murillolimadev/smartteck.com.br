...
echo <<<HTML
			<tr>					
				<td class="">{$parcela}</td>
				<td class="{$classe_pago}">R$ <input $read_only type="text" name="valor" id="valor_{$id_par}" value="{$valor}" style="width:55px; background: transparent; border:none; outline: none; border-bottom:1px solid #000;"></td>				
				<td class="{$classe_venc}"><input $read_only type="date" name="data_venc" id="data_venc_{$id_par}" value="{$data_venc}" style="width:110px; background: transparent; border:none; outline: none; border-bottom:1px solid #000; height:21px"></td>
				...
				<big><a class="{$editar_contas}" href="#" onclick="editar_contas('{$id_par}')" title="Editar Dados"><i class="fa fa-edit text-primary"></i></a></big>
					...
					<big><a class="{$ocultar_baixar}" href="#" onclick="cobrar('{$id}', '{$parcela}', '{$valor_final}', '{$data_venc}', '{$telefone}', '{$valor_multa}', '{$valor_juros}', '{$id_par}')" title="Gerar Cobrança"><i class="fa fa-whatsapp verde"></i></a></big>
...
function editar_contas(id){
	...
	var id_cob = $('#id_cobranca').val();
	 ...
	 $.ajax({
	        url: 'paginas/' + pag + "/editar_valores.php",
	        method: 'POST',
	        data: {id, valor, data},
	        dataType: "html",
